...
function UserpageInfo() {
    ...
    const [bioTextCount, setBioTextCount] = useState(100);
    ...
    const handleBioTextCount = (event) => {
        const inputValue = event.target.value;

        if (inputValue.length > 100) {
            event.preventDefault();
            event.target.setSelectionRange(0, 100);
        } else {
            setBioText(inputValue);
            setBioTextCount(100 - inputValue.length)
        }
    };
    ...
    return (
        <div className='userpageInfo'>
            <div className='userpageInfoTop'>
                <NavLink to="/userhomepage/about" activeclassname="active">
                    <h3>Intro</h3>
                </NavLink>
            </div>

            <div className='userpageInfoBottom'>
                    ...
                    {isBioSectionVisible ? (
                        <div className="bioSection_Top" style={{ display: 'flex' }}>
                            <textarea
                                ...
                                onChange={handleBioTextCount}>
                            </textarea>

                            <p id='limitNote'>{bioTextCount} charcters remaining</p>
                        </div>
                    ) : (
                        <div className="bioText" style={{ display: bioText ? 'block' : 'none' }}>
                            {bioText}
                        </div>
                    )}
